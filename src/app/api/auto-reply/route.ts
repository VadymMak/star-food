...
// Language configs for email subject + AI instruction
...
function detectLanguage(text: string): string {
  // Simple heuristic: check for Cyrillic, Turkish chars, etc.
  if (/[а-яА-ЯёЁ]/.test(text)) {
    if (/[іїєґІЇЄҐ]/.test(text)) return "ua";
    if (/[ъьЪЬ]/.test(text) || /[жзцчшщЖЗЦЧШЩ]/.test(text)) return "bg"; // rough
    return "ru";
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const expectedSecret = process.env.AUTO_REPLY_SECRET || "internal";
    ...
    // Build context about the inquiry
    let inquiryDetails = "";
    if (type === "quote") {
      inquiryDetails = [
        `Type: Quote Request`,
